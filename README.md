...
Для `.mp3`, `.m4a` и `.mp4` нужен `ffmpeg` в `PATH`. Для `.mp4` программа извлекает и транскрибирует только аудиодорожку. Видеоряд не анализируется.
...
## Глобальные настройки, RU/EN и очередь файлов и ссылок

В верхней части интерфейса находится общий блок `Настройки транскрибации`. Выбранные модель Whisper и устройство `Авто`, `CPU` или `GPU / CUDA` применяются к последней записи, выбранному файлу и очереди. При запуске очереди настройки фиксируются в job JSON и не меняются до ее завершения.
...
Раздел `Очередь файлов и ссылок` позволяет смешивать локальные `.wav`, `.mp3`, `.m4a`, `.mp4` и публичные HTTP(S)-ссылки. Для ссылок используется `yt-dlp`: приложение скачивает или извлекает аудио и сохраняет его в:

```text
C:\Python\LocalAudioTranscriber\data\downloads
```

Основной приоритет проверки URL: YouTube, VK Video и RuTube. Остальные публичные источники работают best effort, если их поддерживает установленная версия `yt-dlp` без отдельной авторизации.
...
Транскрипты сохраняются в `data\transcripts` с именами, связанными с исходниками:
