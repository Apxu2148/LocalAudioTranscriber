...
## Очередь локальных файлов

Раздел `Очередь локальных файлов` позволяет выбрать несколько `.wav`, `.mp3`, `.m4a` и `.mp4` файлов. Нажмите `Добавить файлы в очередь`, выберите модель Whisper и нажмите `Запустить очередь`.
...
- ссылки YouTube, RuTube и VK Video пока не поддерживаются;
- скачивание файлов по ссылкам не поддерживается;
